...
test("scaffold examples are isolated and excluded from publicFitments", () => {
  ...
  // Current site: scaffoldExamples may be empty (fail-closed; no published scaffold rows).
...
test("homepage tagline is responsive-site copy (no Every DA16T)", () => {
  const html = fs.readFileSync(path.join(__dirname, "../index.html"), "utf8");
  assert.equal(/Every\s+DA16T/i.test(html), false);
  assert.match(html, /FITMENT DONE RIGHT/);
  assert.match(html, /Find what fits/i);
  assert.match(html, /viewport[^>]*width=device-width/i);
});
